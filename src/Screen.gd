...
#onready var base_size = Globals.BASE_SIZE #Vector2(960, 540)
...
func set_windowed():
	...
	var window_x = (window_size.x / 2) - (scale_x / 2)
	var window_y = (window_size.y / 2) - (scale_y / 2)
	OS.set_window_position(Vector2(window_x, window_y))

	OS.set_borderless_window(false)
	OS.set_window_fullscreen(false)
	var scaled_size = Vector2(base_size.x * scale_x, base_size.y * scale_y)
	OS.set_window_size(scaled_size)
	vp.set_size(scaled_size)
	pass
